...
public class Register extends ThymeleafHTTPServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (!InputStringChecker.checkEmail(email)) {
            response.sendRedirect(new PathBuilder(URLs.AUTH_PAGE).addParam(AttributeNames.ERROR, ErrorParameter.AUTH_INVALID_EMAIL).toString());
            return;
        }
        if (!pass.equals(r_pass) || !InputStringChecker.checkPass(pass)) {
            response.sendRedirect(new PathBuilder(URLs.AUTH_PAGE).addParam(AttributeNames.ERROR, ErrorParameter.AUTH_INVALID_PASSWORD).toString());
            return;
        }
        ...
        try (UserDAO userDAO = new UserDAO()) {
            ...
            if (userDAO.checkUnique(email)) {
                ...
                response.sendRedirect(new PathBuilder(URLs.AUTH_PAGE).addParam(AttributeNames.ERROR, ErrorParameter.AUTH_USER_EXISTS).toString());
            }
